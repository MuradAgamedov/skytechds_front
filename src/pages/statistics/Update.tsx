...
export default function StatisticUpdate() {
  ...
  const fetchStatistic = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Failed to fetch statistic details')
      const data = await response.json()
      ...
      const initialSubtitles: Record<number, string> = {}
      ...
      setIconAltTexts(initialIconAltTexts)

    } catch (err) {
      console.error('Error fetching statistic:', err)
      setErrorMessage('Failed to load statistic details.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to update statistic')
      }
      ...
      await Swal.fire({
        title: 'Error!',
        text: err.message || 'Failed to update statistic.',
        icon: 'error',
  ...
  return (
    <div style={{ padding: '20px' }}>
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: 0, color: '#f9fafb' }}>Update Statistic</h1>
        <p style={{ color: '#6b7280', margin: '4px 0 0 0' }}>Modify an existing statistic item</p>
      </div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ color: '#6b7280', fontSize: '12px', margin: '4px 0 0 0' }}>
                  Upload new icon image (optional)
                </p>
              </div>
            </div>
          </div>
