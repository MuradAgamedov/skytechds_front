...
export default function TestimonialUpdate() {
  ...
  const fetchTestimonial = async (currentLangs: Language[]) => {
    try {
      const apiUrl = import.meta.env?.VITE_API_URL || 'http://127.0.0.1:8000/api'
      const token = localStorage.getItem('auth_token')
      ...
      setPhotoPreview(testimonialData.photo)

      // Initialize translation data - map translations to languages by order
      const newFullNames: Record<number, string> = {}
      const newCompanies: Record<number, string> = {}
      const newPositions: Record<number, string> = {}

      // Map translations to languages by array index order
      if (testimonialData.translations && Array.isArray(testimonialData.translations)) {
        testimonialData.translations.forEach((translation: Translation, index: number) => {
          const langId = currentLangs[index]?.id || translation.id;
          if (langId) {
            newFullNames[langId] = translation.full_name || ''
            newCompanies[langId] = translation.company || ''
            newPositions[langId] = translation.position || ''
          }
        })
      }
      ...
      setErrorMessage('Failed to load testimonial')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to update testimonial: ${text}`)
      }
      ...
      try {
        const jsonError = JSON.parse(msg.replace('Failed to update testimonial: ', ''))
        if (jsonError.errors) {
  ...
  return (
    <div style={{ padding: '20px' }}>
      <div style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '16px' }}>
        ...
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: 0, color: '#f9fafb' }}>Update Testimonial</h1>
          <p style={{ color: '#6b7280', margin: '4px 0 0 0' }}>Edit customer testimonial</p>
        </div>
      </div>
