...
export default function TagUpdate() {
    ...
    const fetchTag = async (currentLangs: Language[]) => {
        try {
            const apiUrl = import.meta.env?.VITE_API_URL || 'http://127.0.0.1:8000/api'
            const token = localStorage.getItem('auth_token')
            ...
            if (tag.translations && Array.isArray(tag.translations)) {
                // Tag translations from API show as objects like { id: 4, title: "test" }
                // We need to map them to language IDs (3, 4, 5 for az, en, ru)
                // Since API doesn't return language_id directly but returns them in order,
                // we map them based on index or assuming they align with the active languages.
                tag.translations.forEach((t: any, index: number) => {
                    // If the backend doesn't return language_id, let's map by index to the languages array
                    const langId = currentLangs[index]?.id;
                    if (langId) {
                        initialTitles[langId] = t.title || ''
                    } else if (t.language_id) {
            ...
            setErrorMessage('Failed to load tag details.')
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            try {
                const jsonError = JSON.parse(msg.replace('Failed to update tag: ', ''))
                if (jsonError.errors) {
    ...
    return (
        <div style={{ padding: '20px' }}>
            <div style={{ marginBottom: '24px' }}>
                <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: 0, color: '#f9fafb' }}>Update Tag</h1>
                <p style={{ color: '#6b7280', margin: '4px 0 0 0' }}>Modify an existing tag</p>
            </div>
