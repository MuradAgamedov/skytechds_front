  ...
  const fetchFaq = async () => {
    ...
    try {
      ...
      if (faq.translations && Array.isArray(faq.translations)) {
        faq.translations.forEach((t: any) => {
          if (t.language_id) {
            initialQuestions[t.language_id] = t.question || ''
            initialAnswers[t.language_id] = t.answer || ''
          }
        })
      }
      ...
      setErrorMessage('Failed to load FAQ details.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div style={{ padding: '20px' }}>
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: 0, color: '#f9fafb' }}>Update FAQ</h1>
        <p style={{ color: '#6b7280', margin: '4px 0 0 0' }}>Modify an existing frequently asked question</p>
      </div>
