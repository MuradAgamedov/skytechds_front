...
export default function MapUpdate() {
  ...
  useEffect(() => {
    ...
    const fetchMap = async () => {
      try {
        ...
        const res = await fetch(`${apiUrl}/admin/maps/${id}`, {
          method: 'GET',
          headers: {
        ...
        setFormData({
          address: map.map,
          status: map.status === 1 ? 'active' : 'inactive'
        })
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!id) {
      setErrorMessage('No map ID provided')
      return
    }
    setErrorMessage('')
    setSubmitting(true)
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to update map: ${response.status} ${text}`)
      }
      
      console.log('Map updated successfully:', payload)
      
      await Swal.fire({
        title: 'Success!',
        text: 'Map has been updated successfully.',
        icon: 'success',
      ...
      console.error('Error updating map:', msg)
      setErrorMessage(msg)
  ...
  return (
    <div style={{ padding: '20px' }}>
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: 0 }}>Update Map</h1>
        <p style={{ color: '#6b7280', margin: '4px 0 0 0' }}>Edit map information</p>
      </div>
