  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to create email: ${response.status} ${text}`)
      }
      
      await Swal.fire({
        title: 'Success!',
        text: 'Email has been created successfully.',
        icon: 'success',
      ...
      console.error('Error creating email:', msg)
      setErrorMessage(msg)
  ...
  return (
    <div style={{ padding: '20px' }}>
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: 0 }}>Create Email</h1>
        <p style={{ color: '#6b7280', margin: '4px 0 0 0' }}>Add new email address</p>
      </div>
