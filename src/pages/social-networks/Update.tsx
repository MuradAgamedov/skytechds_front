...
export default function SocialNetworkUpdate() {
  ...
  const fetchSocialNetwork = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/admin/social-networks/${id}`, {
        method: 'GET',
        headers: {
      ...
      if (!res.ok) throw new Error('Failed to fetch social network')
      const json = await res.json()
      const socialNetwork = json.data
      
      setFormData({
        platform: socialNetwork.platform,
        url: socialNetwork.url,
        status: socialNetwork.status === 1 ? 'active' : 'inactive'
      })
    } catch (err) {
      console.error('Error loading social network:', err)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!id) {
      setErrorMessage('No social network ID provided')
      return
    }
    setErrorMessage('')
    setSubmitting(true)
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to update social network: ${response.status} ${text}`)
      }
      
      console.log('Social network updated successfully:', payload)
      
      await Swal.fire({
        title: 'Success!',
        text: 'Social network has been updated successfully.',
        icon: 'success',
      ...
      console.error('Error updating social network:', msg)
      setErrorMessage(msg)
  ...
  return (
    <div style={{ padding: '20px' }}>
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: 0 }}>Update Social Network</h1>
        <p style={{ color: '#6b7280', margin: '4px 0 0 0' }}>Edit social network link</p>
      </div>
      ...
      <div style={{ backgroundColor: '#1f2937', borderRadius: '8px', border: '1px solid #374151', padding: '24px', maxWidth: '800px' }}>
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            >
              {submitting ? 'Updating…' : 'Update Social Network'}
            </button>
            <button
              type="button"
