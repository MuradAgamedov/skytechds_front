...
export default function BlogCreate() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        const jsonError = JSON.parse(msg.replace('Failed to create blog: ', ''))
        if (jsonError.errors) {
  ...
  return (
    <div style={{ padding: '20px' }}>
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: 0, color: '#f9fafb' }}>Create Blog</h1>
        <p style={{ color: '#6b7280', margin: '4px 0 0 0' }}>Add a new blog post</p>
      </div>
